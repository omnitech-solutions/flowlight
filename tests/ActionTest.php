...
describe('::execute', function () {
    ...
    it('short-circuits when context is already complete (no hooks, no perform)', function () {
        $calls = [];
        ...
        $Fake = new class extends Action
        {
            protected function perform(Context $ctx): void
            {
                // If this ever runs, the guard failed
                throw new RuntimeException('perform() should not run when context is complete');
            }
        };

        // Wire hooks to prove none are invoked (must be callable(Context): void)
        $Fake::setBeforeExecute(function (Context $c) use (&$calls): void {
            $calls[] = 'before';
        });
        $Fake::setAfterExecute(function (Context $c) use (&$calls): void {
            $calls[] = 'after';
        });
        $Fake::setAfterSuccess(function (Context $c) use (&$calls): void {
            $calls[] = 'success';
        });
        $Fake::setAfterFailure(function (Context $c) use (&$calls): void {
            $calls[] = 'failure';
        });

        $given = Context::makeWithDefaults(['seed' => 1])->markComplete();

        $out = $Fake::execute($given);

        expect($out)->toBe($given)               // returns same instance
            ->and($out->isComplete())->toBeTrue()// still complete
            ->and($out->errors()->isEmpty())->toBeTrue()
            ->and($calls)->toBe([]);             // no hooks called
    });
});
